...
beatAML <- PharmacoGx::downloadPSet(name = 'BeatAML_2018', 
                           saveDir = getwd(), 
...
to_remove <- unique(c(names(which(apply(gex, 2, function(x) sum(is.na(x)) > ncol(gex)/10))), 
...
dat <- list('mut' = mut, 'gex_gs' = gex_gs, 
