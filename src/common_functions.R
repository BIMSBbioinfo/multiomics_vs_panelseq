...
fit_RSF_clin <- function(clin, M, ntree = 1000) {
  ...
  fit3 <- randomForestSRC::rfsrc(formula = Surv(time, status) ~ .,
                                 data = subset(dat, select = c('time', 'status',
                                                               setdiff(colnames(dat),
                                                                     colnames(clin)))), 
                                 ntree = ntree)
  Cindex3 <- round(1 - mean(fit3$err.rate, na.rm = T), 2)
  
  return(list('fit_with' = fit1, 'fit_wo' = fit2, 'fit_only_LFs' = fit3,
              'C_with' = Cindex1, 'C_wo' = Cindex2, 'C_only_LFs' = Cindex3))
}
...
# for each feature, build a univariate cox model, 
# return features with significant HR < 1 or HR > 1
select_features_univariate_cox <- function(surv, features, M, pval_threshold = 0.05) {
  require(data.table)
  require(survival)
  df <- as.data.frame(t(M[features,]))
  df$PFI <- surv[match(rownames(df), bcr_patient_barcode)]$PFI
  df$PFI.time <- surv[match(rownames(df), bcr_patient_barcode)]$PFI.time
  warning("converting dashes in feature names to dots")
  colnames(df) <- gsub('-', '.', colnames(df))
  dt <- do.call(rbind, lapply(features, function(f) {
    fit <- coxph(as.formula(paste('Surv(PFI.time, PFI) ~ ',f)), 
                 data = df)
    s <- summary(fit)
    coefs <- s$coefficients
    confs <- s$conf.int
    data.table('feature' = f, 'pval' = as.numeric(coefs[,5]), 
               'coef' = as.numeric(coefs[,2]), 
               'direction' = ifelse(coefs[,'z'] < 0, 'beneficial', 'risky'))
  }))
  return(dt[order(pval)][pval < pval_threshold])
}
# use pensim package to do nested cross-validation of survival prediction using lasso 
get_nested_cv_risk_scores <- function(surv, M) {
  require(penalized)
  require(pensim)
  surv.df <- get_surv_df(surv, M)
  surv.obj <- survival::Surv(surv.df$time, surv.df$status)
  dat.filt <- surv.df[,1:ncol(M)]
  set.seed(1)
  preds <- opt.nested.crossval(outerfold = 5, nprocessors = 5, optFUN = "opt1D", scaling = FALSE,
                               setpen = "L1", nsim = 50, response = surv.obj, penalized = dat.filt, 
                               fold = 5, positive = FALSE, standardize = TRUE, trace = FALSE)
  return(list('surv_obj' = surv.obj, 'preds' = preds))
}
