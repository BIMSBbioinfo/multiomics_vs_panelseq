...
res[["CCLE_varImp"]] %>%
  ...
  dplyr::slice(1:100) %>%
  summarise(
    cts.gex = sum(str_count(Feature, "gex")),
...
ggplot(
  ...
  geom_vline(
    xintercept = c(0, mean(t.plot.varImp$mo - t.plot.varImp$panel, na.rm = T)),
    linetype = c("solid", "dashed"), color = c("black", "black"), size = c(0.5, 1)
  ) +
  geom_text(
    data = tibble(
      x = mean(t.plot.varImp$mo - t.plot.varImp$panel, na.rm = T) + 0.08, y = 1.5,
      text = paste0(
        "Mean improvement = ",
